...
const PLAYER_POSES: Record<PremiumPlayerPose, string> = {
  "turn-ready": "/characters/premium/player/turn-ready.png",
  "card-play": "/characters/premium/player/card-play.png",
  "light-cast": "/characters/premium/player/light-cast.png",
  "heavy-cast": "/characters/premium/player/heavy-cast.png",
  hurt: "/characters/premium/player/hurt.png",
  frozen: "/characters/premium/player/frozen.png",
  burn: "/characters/premium/player/burn.png",
  wind: "/characters/premium/player/wind.png",
  "final-card": "/characters/premium/player/final-card.png",
  victory: "/characters/premium/player/victory.png",
  defeat: "/characters/premium/player/defeat.png"
};

const textureKey = (pose: PremiumPlayerPose) => `premium-player:${pose}`;

export function preloadPremiumPlayer(scene: Phaser.Scene): void {
  for (const [pose, path] of Object.entries(PLAYER_POSES) as [PremiumPlayerPose, string][]) {
    scene.load.image(textureKey(pose), path);
  }
}

export class PremiumPlayerDirector {
  ...
  private persistentPose: PremiumPlayerPose = "turn-ready";
  ...
  setPersistentPose(pose: PremiumPlayerPose): void {
    this.persistentPose = pose;
    if (!this.restoreTimer) this.show(pose);
  }

  play(pose: PremiumPlayerPose, duration = 900): void {
